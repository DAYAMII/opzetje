...
var part = 10;  //particle 
var meshW = 90;  //mesh width
var dispX = -5;  //x disposition
var dispY = -10;  //y disposition
var partX = 100;  //particle x
var partY = 0;  //particle y
var partIndX = 1;  //particle index x
...
var int = setInterval(intHandler, 1000 / 50);
...
function mouseDown() {
 if (int != undefined) {
   ...
   int = setInterval(intHandler, 800 / 30);
 }
}
...
function Object(pX, pY, pIndX, pIndY) {
 ...
 this.tracker = (Math.PI / 4) + this.indX * .5;
 this.diffX = Math.random();
 this.diffY = Math.random();
 this.speed = .1;
 this.vol = 60;  //volume  (higher #, more movement)
 ...
 this.colRng = (200 - this.colRngDiff) + Math.floor(Math.random() * this.colRngDiff);
 ...
 this.fill = function () {
   if (this.connectAll.length > 0) {
     this.calcW = Math.abs(this.connectAll[2].x - this.x);
